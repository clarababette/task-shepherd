...
const getCoderTask = async (req, res) => {
  ...
  const getCoderTasks = async (req, res) => {
    ...
    const query = 'select assigned_tasks.id, tasks.name, assigned_tasks.status from assigned_tasks join coders on assigned_tasks.coder_id=coders.id join tasks on assigned_tasks.task_id = tasks.id where coder_id=$1 sort by assigned_tasks.status_timestamp';
